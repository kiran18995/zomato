...
public class RestaurantPageKeyedDataSource extends PageKeyedDataSource<Integer, Restaurant> {
    ...
    public RestaurantPageKeyedDataSource(RestaurantService restaurantService) {
    ...
    public void loadAfter(@NonNull final LoadParams<Integer> params, @NonNull final LoadCallback<Integer, Restaurant> callback) {
        restaurantService.getRestaurants().enqueue(new Callback<RestaurantResponse>() {
            ...
            public void onResponse(Call<RestaurantResponse> call, Response<RestaurantResponse> response) {
                if (response.isSuccessful()) {
                    callback.onResult(response.body().getRestaurants(), params.key + 1);
